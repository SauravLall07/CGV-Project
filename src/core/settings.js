...
// Tone-mapping exposure the scene lighting and PBR materials are balanced
// around; the Brightness slider moves off this and reads 100% here.
const DEFAULT_BRIGHTNESS = 0.95
...
export const OPTION_DEFS = [
  {
    ...
    max: 2.2,
    step: 0.05,
    // 100% is the exposure the lighting and materials were balanced against,
    // so the readout is relative to the default rather than absolute.
    format: (v) => `${Math.round((v / DEFAULT_BRIGHTNESS) * 100)}%`
  },
  {
    id: 'fov',
    label: 'Field of View',
